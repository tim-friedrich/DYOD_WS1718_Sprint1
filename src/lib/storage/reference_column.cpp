...
namespace opossum {
  ReferenceColumn::ReferenceColumn(const std::shared_ptr<const Table> referenced_table, const ColumnID referenced_column_id,
                  const std::shared_ptr<const PosList> pos) : _referenced_table(referenced_table), _referenced_column_id(referenced_column_id), _pos(pos){

  }

  const AllTypeVariant ReferenceColumn::operator[](const size_t i) const{
    RowID row = _pos->at(i);
    return (*_referenced_table->get_chunk(row.chunk_id).get_column(_referenced_column_id))[row.chunk_offset];
  }
